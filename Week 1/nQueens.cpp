...
void search (unsigned row, std::vector<unsigned> & queens, std::vector< std::vector<unsigned> > & threats, unsigned n) 
{
    ...
    else 
    {
        ...
        {
            // only place a queen in a column on this row that is not threatened.
            if( 0 == threats[row][ queens[row] ] )
            {

                // place a queen on this spot:
                changeThreats(row, queens[row], +1, threats, n);

                // place queens on the remaining rows:
                search(row+1, queens, threats, n);

                // remove the queen from this spot:
                changeThreats(row, queens[row], -1, threats, n);
            }
        }
    }
...
void changeThreats(unsigned row, unsigned column, int change, std::vector< std::vector<unsigned> > & threats, unsigned n)
{
    ...
    {
        // fill in the updated threats in row j.
        ...
        if( column + (j-row) < n )
        {
            threats[j][column + (j-row) ] += change;
        }

        if( column >= (j-row) ) // careful:  don't do column - (j-row) >= 0 -- that's always true!
        {
            threats[j][column - (j-row)] += change;
        }
