...
void printTiling(Point topLeft, unsigned n, Point missingPiece)
{
	...
	unsigned quMissing = whichQuadrant( topLeft, n, missingPiece );
	if( 2 == quMissing )
	{
		Point inCo = getInternalCorner( topLeft, n, 1);
		placeTile(2, inCo);
	}
	else
	{
		Point inCo = getInternalCorner( topLeft, n, 2);
		placeTile(quMissing, inCo);
	}

	for(unsigned qu = 1; qu <= 4; qu++)
	{
		// In order to have a recursive call, I need to know which "board" it is:  top-Left and size.
		// size will be n-1, but where is the top left?
		Point topl = getTopLeftOfQuadrant(topLeft, n, qu);

		// And I need to give it a missing piece (that is a problem constraint)
			// If this quadrant is where the original missing piece is, that's the one to use.
			// Otherwise, the fragment of the piece placed above is the "missing piece"
		Point missing = (quMissing == qu) ? missingPiece : getInternalCorner( topLeft, n, qu);

		// And now just make the recursive call. 
		printTiling(topl, n-1, missing);
	}
